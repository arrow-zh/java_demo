...
cBoard.controller('paramCtrl', function ($scope, $uibModal, $http) {
    ...
    $scope.init = function (crow) {
        ...
        if($scope.param.paramType == 'search') {
            $scope.param.type = 'like';
        } else if($scope.param.paramType == 'eq') {
            $scope.param.type = '=';
        } else if($scope.param.paramType == 'select'){
            $scope.param.type = '=';
        } else if($scope.param.paramType == 'checkbox'){
            $scope.param.type = '=';
        }

        if(!$scope.param.type) $scope.param.type = 'selector';
    ...
    $scope.clickCheckBox = function (crow) {
        console.log(crow)
        _.each(crow.paramRows, function (paramRow) {
            _.each(paramRow.params, function (param) {
                if(param.paramType == 'checkbox') {
                    param.values = [];
                    _.each(param.selects, function (value) {
                        if(param.checks[value]) {
