...
cBoard.controller('dashboardViewCtrl', function ($timeout, $rootScope, $scope, $state, $stateParams, $http, ModalUtils, chartService, $interval, $uibModal, dataService, dateSelectService) {
    ...
    var loadWidget = function (reload) {
        $scope.pathname = window.location.pathname.indexOf("share.html");
    ...
    var paramToFilter = function () {
        ...
        //将点击的参数赋值到看板上的参数中
        // {"params":[{"targetField":"name", "type":"like","value":"康"}]}
        ...
        console.log($scope.board.layout.rows)
    ...
    $scope.search = function (crow) {
        $scope.applyParamFilter(crow);
    };

    $scope.reset = function (crow) {
        _.each(crow.paramRows, function (paramRow) {
            _.each(paramRow.params, function (param) {
                if(param.paramType != 'url') {
                    param.values = [];
                }
